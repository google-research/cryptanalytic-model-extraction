...
def is_on_following_layer(known_T, known_A, known_B, point):
    logger.log("Check if the critical point is on the next layer", level=Logger.INFO)
    ...
    if is_on_prior_layer(point):
        logger.log("It's not, because it's on an earlier layer", level=Logger.INFO)
        return False
    ...
    for tol in range(10):
        ...
        if CHEATING:
            logger.log(cheat_get_inner_layers(point + go_direction * high / 2)[np.argmin(ls)], level=Logger.INFO)

        point_in_same_polytope = point + (high * .999 - 1e-4) * go_direction

        logger.log("high", high, level=Logger.INFO)
        ...
        solutions = do_better_sweep(point_in_same_polytope,
                                    ...
                                    known_T=known_T)
        if len(solutions) >= 1:
            logger.log("Correctly found", len(solutions), level=Logger.INFO)
        else:
            return False
    ...
    if CHEATING:
        ...
        logger.log(soln, level=Logger.INFO)
...
def find_plane_angle(known_T,
    ...
    for stepsize in start + list(range(init_step, MAX)):
        logger.log("\tTry stepping away", stepsize, level=Logger.INFO)
        ...
        if CHEATING:
            logger.log("\tHAVE BOX INTERSECT COUNT", len(intersections), level=Logger.INFO)
            logger.log("\t", len(left), len(right), len(top), len(bot), level=Logger.INFO)
        ...
        if (len(intersections) == 0 and stepsize > 15):  # or (len(intersections) == 3 and stepsize > 5):
            ...
            logger.log("\tIt looks like we're in a flat region, raise failure", level=Logger.INFO)
            raise AcceptableFailure()
        ...
        if len(intersections) > 4 and len(prev_iter_intersections) < 2:
            logger.log("\tWe didn't get enough inner points", level=Logger.INFO)
            if exponential_base == 1.2:
                logger.log("\tIt didn't work a second time", level=Logger.INFO)
                return None, None, 0
            else:
                logger.log("\tTry with smaller step", level=Logger.INFO)
        ...
        if (len(intersections) > 4 or stepsize > 20) and len(prev_iter_intersections) >= 2:
            ...
            if sorted(candidate)[3][0] < 1e-8:
                ...
                logger.log("\tFailed the box continuation finding procedure. (1)", level=Logger.INFO)
                logger.log("\t", candidate, level=Logger.INFO)
                raise AcceptableFailure()
            ...
            for critical_point, is_prior_layer_zero in zip(intersections, prior_layer_near_zero):
                ...
                if is_prior_layer_zero:
                    ...
                    if all([np.min(np.abs(x)) > 1e-5 for x in vs]):
                        ...
                        logger.log("\tAbort 1: failed to find a valid box", level=Logger.INFO)
                        should_fail = True
                if any([np.min(np.abs(x)) < 1e-10 for x in vs]):
                    # We expect the prior layer to be zero.
                    if not is_prior_layer_zero:
                        # If it looks like it's not actually zero, then brutally fail.
                        logger.log("\tAbort 2: failed to find a valid box", level=Logger.INFO)
                        should_fail = True
            if should_fail:
                return None, None, 0
            ...
            for critical_point, is_prior_layer_zero in zip(intersections, prior_layer_near_zero):
                sign_at_crit = sign_to_int(get_polytope_at(known_T,
                                                           known_A, known_B,
                                                           critical_point))
                logger.log("\tMove to", sign_at_crit, 'versus', sign_at_init, is_prior_layer_zero, level=Logger.INFO)
                if not is_prior_layer_zero:
                    if sign_at_crit != sign_at_init:
                        success = critical_point
                        if CHEATING:
                            logger.log('\tinner at success', cheat_get_inner_layers(success), level=Logger.INFO)
                        logger.log("\tSucceeded", level=Logger.INFO)
                    else:
            ...
            if success is None:
                logger.log("\tFailed the box continuation finding procedure. (2)", level=Logger.INFO)
                raise AcceptableFailure()
...
def binary_search_towards(known_T, known_A, known_B, start_point, initial_signs, go_direction, maxstep=1e6):
    ...
    delta_hidden_np = (np.array(
        plus_T.get_hidden_layers(start_point + 1e-6 * go_direction, flat=True)) - initial_hidden) * 1e6
    #
    ...
    if len(can_go_dist) == 0:
        logger.log("Can't go anywhere at all", level=Logger.INFO)
        raise AcceptableFailure()
...
def is_solution(input_tuple):
    ...
    if CHEATING:
        if signs % 1001 == 0:
            logger.log('tick', signs, level=Logger.INFO)
    else:
        if signs % 100001 == 0:
    ...
    if np.any(np.isnan(mat)) or np.any(np.isinf(mat)):
        logger.log("Invalid solution", level=Logger.INFO)
        return (res, new_signs, solution), 0
    else:
...
def solve_contractive_sign(known_T, weight, bias, LAYER):
    logger.log("Solve the extraction problem for contractive networks", level=Logger.INFO)
    ...
    for axis in range(len(hidden)):
        ...
        logger.log("Confirm preimage", level=Logger.INFO)
        ...
        logger.log(standard_out, out_plus, out_minus, level=Logger.INFO)

        inverted_if_small = np.sum(np.abs(out_plus - standard_out))
        not_inverted_if_small = np.sum(np.abs(out_minus - standard_out))

        logger.log("One of these should be small",
                   inverted_if_small,
                   not_inverted_if_small, level=Logger.INFO)
