...
def check_quality(layer_num, extracted_normal, extracted_bias, do_fix=False):
    ...
    logger.log("\nCheck the solution of the last weight matrix.", level=Logger.INFO)
    ...
    for i in range(neuron_count[layer_num + 1]):
        ...
        for j in range(neuron_count[layer_num + 1]):
            ...
            gaps.append(error)
            ratios.append(ratio)
        logger.log("Neuron", i, "maps on to neuron", np.argmin(gaps), "with error", np.min(gaps) ** .5, 'ratio',
                   ratios[np.argmin(gaps)], level=Logger.INFO)
        logger.log("Bias check", (__cheat_B[layer_num][np.argmin(gaps)] - extracted_bias[i] * ratios[np.argmin(gaps)]),
                   level=Logger.INFO)
        ...
        if min(gaps) > 1e-2:
            logger.log("ERROR LAYER EXTRACTED INCORRECTLY", level=Logger.INFO)
            logger.log("\tGAPS:", " ".join("%.04f" % x for x in gaps), level=Logger.INFO)
            logger.log("\t  Got:", " ".join("%.04f" % x for x in extracted_normal[:, i] / extracted_normal[0, i]),
                       level=Logger.INFO)
            logger.log("\t Real:", " ".join("%.04f" % x for x in
                                            __cheat_A[layer_num][:, np.argmin(gaps)] / __cheat_A[layer_num][
                                                0, np.argmin(gaps)]), level=Logger.INFO)
